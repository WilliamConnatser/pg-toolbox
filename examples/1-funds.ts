...
import { ToolBoxFile } from 'pg-toolbox/types'

const fundsToolBoxFile: () => Promise<ToolBoxFile> = async () => ({
  migrate: slonik.sql`CREATE TABLE funds (
      id  SERIAL PRIMARY KEY,
      name VARCHAR(50) NOT NULL UNIQUE,
      ticker  VARCHAR(5) NOT NULL UNIQUE,
      url VARCHAR(150) NOT NULL UNIQUE
    )`,
  rollback: slonik.sql`DROP TABLE IF EXISTS funds`,
  truncate: slonik.sql`TRUNCATE funds RESTART IDENTITY CASCADE`,
  seed: slonik.sql`INSERT INTO funds (name, ticker, url)
  SELECT *
